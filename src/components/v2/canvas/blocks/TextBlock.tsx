...
export function TextBlock({ block }: { block: CanvasBlock }) {
  const text = block.content ?? "";

  return (
    <div
      className="prose prose-sm max-w-none prose-headings:font-bold prose-p:leading-relaxed prose-a:font-medium prose-a:no-underline prose-code:before:content-none prose-code:after:content-none px-4 py-3"
    ...
    >
      <Markdown
        ...
        components={{
          pre: ({ children }) => (
            <pre
              className="overflow-x-auto rounded-xl p-3 text-[12px] text-white"
          ...
          table: ({ children }) => (
            <div
              className="my-2 overflow-x-auto rounded-xl"
