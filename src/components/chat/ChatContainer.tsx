...
/* ═══════════════════════════════════════════════
   ChatContainer — mock-server streaming
   ═══════════════════════════════════════════════ */
...
export function ChatContainer({ chatId }: ChatContainerProps) {
            ...
            metadata:
              m.role === "assistant"
                ? {
                    ...
                    queryExecuted: m.canvas
                      ? generateMockSql(rawMessages[i - 1]?.content || "")
                      : undefined,
                  }
                : undefined,
          }));
          setMessages(
            loaded.filter((m) => m.role === "user" || m.role === "assistant")
          );

          const lastWithCanvas = [...rawMessages]
            .reverse()
            .find((m: any) => m.canvas);
  ...
  const handleRegenerate = useCallback(
    (messageId: string) => {
      ...
      doSendMessage(userMsg.content);
    },
    [messages, doSendMessage]
  );

  return (
    <div className="flex h-full flex-col" style={{ background: "var(--ci-bg)" }}>
      <div ref={scrollRef} className="flex-1 overflow-y-auto ci-scrollbar">
        {messages.length === 0 && !isStreaming ? (
          ...
          <div className="mx-auto max-w-3xl space-y-6 px-4 py-6">
            ...
            {error && !isStreaming && (
              <div
                className="mx-auto max-w-md rounded-xl px-4 py-3 text-center text-[12px]"
                style={{
